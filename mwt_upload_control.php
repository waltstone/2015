...
?>
	<?php if (isset($result)){ //判斷上傳結果?>
        <script language = "javascript">	
            ...
            window.opener.document.getElementById("<?php echo $_POST['prevImg']; ?>").width = 800;
            window.opener.document.getElementById("<?php echo $_POST['formName']; ?>").<?php echo $_POST['rePic']; ?>.value = '<?php echo $file_name; ?>';
            window.close();
        </Script>
    <?php }else{ printf("<b style='color:red;'>上傳失敗:</b><hr>%s",$checkmsg);echo $_FILES['Filedata']['size']; }?>
